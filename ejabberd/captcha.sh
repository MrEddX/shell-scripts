...
# This script is a modified version of the original "https://github.com/processone/ejabberd/blob/master/tools/captcha.sh" script.
# It generates random color but only in the pale color range.
# It takes the text to recognize in the captcha image as a parameter.
# It returns the image binary as a result. ejabberd support PNG, JPEG and GIF.
